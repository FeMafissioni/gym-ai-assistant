...
import {
  GetResumoPosTreinoRequest,
  GetResumoPosTreinoResult,
  ResumoPosTreinoDestaque,
  ResumoPosTreinoExecucao,
  ResumoPosTreinoExercicio,
} from "./types/getResumoPosTreino.types";
...
function buildExecucao(peso: number, repeticoes: number): ResumoPosTreinoExecucao {
  return {
    peso,
    repeticoes,
...
function toDestaque(exercicio: ResumoPosTreinoExercicio): ResumoPosTreinoDestaque | null {
  if (!exercicio.atual || !exercicio.anterior || !exercicio.variacao) return null;
...
export class GetResumoPosTreinoUseCase {
  async execute(
    request: GetResumoPosTreinoRequest
  ): Promise<GetResumoPosTreinoResult> {
    const { userId, sessaoId } = request;

    const sessao = await prisma.sESSAO_TREINO.findFirst({
      where: {
        ID: sessaoId,
        USER_ID: userId,
        FINALIZADO_EM: {
    ...
    const [treinoExercicios, sessaoAnterior] = await Promise.all([
      prisma.tREINO_EXERCICIO.findMany({
        where: {
    ...
    const currentExecucoesByExercicio = new Map(
      sessao.EXECUCOES.map((execucao: ExecucaoBanco) => [execucao.EXERCICIO_ID, execucao])
    );
    const previousExecucoesByExercicio = new Map(
      (sessaoAnterior?.EXECUCOES ?? []).map((execucao: ExecucaoBanco) => [execucao.EXERCICIO_ID, execucao])
    );
    ...
    const exercicios: ResumoPosTreinoExercicio[] = treinoExercicios.map((treinoExercicio) => {
      ...
      let status: ResumoPosTreinoExercicio["status"] = "MANTEVE";
    ...
    const melhorias = exercicios
      ...
      .map(toDestaque)
      .filter((destaque): destaque is ResumoPosTreinoDestaque => destaque !== null)
