...
exports.formatResumoPosTreino = formatResumoPosTreino;
function formatSigned(value) {
    if (value > 0)
        return `+${value}`;
    return `${value}`;
}
function formatDestaqueLinha(tipo, destaque) {
...
function formatResumoPosTreino(resumo) {
    const linhas = [
    ...
    if (!resumo.comparativo.possuiHistoricoAnterior) {
        ...
        linhas.push(`📌 Comparativo: ${resumo.comparativo.melhorou} melhoraram, ${resumo.comparativo.manteve} mantiveram, ${resumo.comparativo.piorou} caíram`);
        if (resumo.comparativo.semHistorico > 0) {
            linhas.push(`ℹ️ ${resumo.comparativo.semHistorico} exercício(s) sem histórico de comparação.`);
        }
    }
